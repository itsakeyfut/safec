...
impl Lowering<'_> {
    ...
    /// belongs here. An integer operand is promoted before an arithmetic
    /// operation, and folding one away would skip a conversion the IR models;
    /// what that costs is a **value**, not a report, and nothing in the suite
    /// would say so. A pointer is not promoted, so the question does not arise
    /// for the case this is for. See ADR-0021.
    ///
    ...
    /// to give `1 - p` a type. RK-042 in the review knowledge bank is the
    /// entry about reading one for the other.
