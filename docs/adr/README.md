...
| [0021](./0021-fold-a-zero-pointer-offset-where-the-ir-is-built.md) | Fold a zero pointer offset where the IR is built, so no analysis sees two shapes for one expression | accepted | Five mutations in `crates/safec/src/lowering.rs`, each failing named tests and nothing else: `unmoved` answering `None` fails the three cases that hold the four spellings agreeing and the three older subscript cases on their IR; dropping its question about the type fails `a_zero_added_to_an_integer_keeps_its_operation`, which is the one guard here whose absence costs a value rather than a report; asking about the subscript's type rather than the base's fails the three older subscript cases; and reading the zero on one side of `Add` only, or not answering for `Sub`, fails one of the new cases each. Every diagnostic in the corpus is byte-identical to what the rejected option produced |

**By status**: accepted: 0001, 0002, 0003, 0004, 0005, 0006, 0007, 0008, 0009, 0010, 0011, 0012, 0013, 0014, 0015, 0016, 0017, 0018, 0019, 0020, 0021 · proposed: none · superseded: none
