...
| [0022](./0022-say-where-c-sequences-one-evaluation-before-another.md) | Say where C sequences one evaluation before another, as an element of a block | accepted | `error[E0004]` at every reader of `Element` if the kind goes, and `error[E0063]` at `Freeing::new` and `Freeing::joined` if a field is added to the pair a free carries. Mutations, each failing named tests and nothing else: `Builder::sequenced` building nothing, and the check not reading the kind, each fail every proved double free and use after free in the corpus and in `freed.rs`; the comma's marker no longer asking what encloses it fails `an_unsequenced_free_and_use_is_not_proved`, `the_same_program_with_the_operands_swapped_is_not_proved_either` and `a_comma_inside_an_unsequenced_operand_is_not_one`, which is the row that matters because the mutation makes the compiler certain rather than quiet; `sequences` answering `false` for a comma fails `a_comma_at_the_top_of_a_full_expression_is_a_sequence_point`; dropping the marker for `,`, `&&`, `||` or `?:` fails the case named for that operator; and `Freeing::joined` taking the stronger flag fails `a_free_sequenced_on_one_arm_only_is_not_a_proof`. `Analysis::height` is held by nothing and the record says so |
