...
| [0016](./0016-an-analysis-is-a-trait-and-an-unreached-block-has-no-value.md) | An analysis is a trait, and a block nothing has reached has no value | accepted | `the_back_edge_changes_the_answer_after_the_loop` in `crates/safec-ir/tests/written.rs`: cutting the solver to one visit per block fails it and fails nothing else, and so does a `join` that always answers `false`. `a_block_nothing_reaches_has_no_answer` fails if an unreachable block is given `on_entry` instead of nothing, and `what_a_loop_writes_and_what_comes_before_it_are_answered_apart` fails if the first arrival at a block is joined into `on_entry` rather than stored. A fifth method on `Analysis` without a default is `error[E0046]` at every implementation |
