...
| [0024](./0024-a-proof-survives-while-the-set-it-is-about-does-not-grow.md) | A proof survives while the set it is about does not grow, and the lattice's join is a second method | accepted | `error[E0027]` at `Held::joined` and `Held::accumulated`, which name every field, so a fact added to `Held` answers for what it means in each role. Four mutations in `crates/safec-ir/src/memory.rs`, each failing named tests and nothing else: `accumulated` joining the proof the way `joined` does fails `a_free_after_an_offset_that_kept_the_set_is_proved`; taking the proof from whichever side has one, and reading `grows` and `shrinks` after the union rather than before, each fail `an_offset_that_grew_the_set_is_not_proved`, which is the row-6 direction; and `joined` accumulating the proof fails `a_free_of_a_may_set_on_one_arm_only`. Dropping the proof at a write through a pointer is held by nothing, because ADR-0017's escape answers `Unknown` there whatever the field says, and the record says so |
