...
| [0023](./0023-carry-a-read-forwards-to-the-free-it-is-unordered-against.md) | Carry a read forwards to the free it is unordered against, as a field of the memory lattice | accepted | `error[E0027]` at `Allocations::join` and `Known::reborn`, which name every field of `Known`, so a field added to that value answers for what a merge and a rebirth do to it. Mutations in `crates/safec-ir/src/memory.rs` and `crates/safec/src/lowering.rs`, each failing named tests and nothing else: recording nothing at either transfer, recording nothing at the terminator, and concluding `Unsafe` rather than `Unknown` each fail `an_unsequenced_use_the_check_meets_first_is_reported` and `a_condition_read_through_a_pointer_in_an_unsequenced_operand_is_reported`; not comparing the sites fails `a_use_of_another_pointer_before_a_free_is_not_reported`; and leaving `pending` alone in `Known::reborn` fails `a_read_of_a_site_handed_to_a_second_allocation_is_not_carried_to_its_free`. Not clearing at a sequence point fails `a_comma_that_frees_after_it_reads`, which is a program C17 6.5.17 p2 defines. An arm that does not begin with the marker fails `a_condition_read_through_a_pointer_is_sequenced_before_the_body` and `..._before_the_loop_exits`, and only the taken arm beginning with it fails `..._before_the_other_arm`, measured on its `.stderr`. `a_use_and_a_free_on_two_arms_of_one_conditional_are_not_both_reached` is held by no mutation of a line and the record says so. `Analysis::height` is held by nothing and the record says so |
