...
occurs. So `int f(int *p) { free(p); free(p); }` is well defined for the caller
that passes a null pointer, and the memory check calls it a proved double free
at every safety level. The same clause exempts `int *p = malloc(4); free(p);
free(p);` on the execution where the allocation failed, because C17 7.22.3.4 p3
...
unit contains: `int f(int *p) { free(p); free(p); }` is a proved double free
however `f` is called here, and stays one in the program whose `main` passes `0`.
...
`Allocations::touching` already applies to `free(0)` with this clause quoted, and
extending it to a local the nullability analysis proved null is the seam #188
builds. Until #188 lands, such a free is reported as a pointer this check stopped
